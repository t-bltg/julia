...
static Value *emit_datatype_mutabl(jl_codectx_t &ctx, Value *dt)
{
    ...
    Value *Idx2 = ConstantInt::get(T_size, offsetof(jl_typename_t, flags));
...
{
    Value *isconcrete;
    isconcrete = ctx.builder.CreateConstInBoundsGEP1_32(T_int8, emit_bitcast(ctx, decay_derived(ctx, typ), T_pint8), offsetof(jl_datatype_t, flags));
